...
# LIB_DIRS := $(foreach lib,$(wildcard lib/*/),$(patsubst %/,%,$(lib)))
# LIB_ENTITIES := $(foreach lib,$(wildcard lib/*/),$(notdir $(patsubst %.cpp,%,$(wildcard $(lib)/src/*.cpp))))

SRCS := $(foreach ent,$(ENTITIES),src/$(ent).cpp)
TEST_SRCS := $(foreach ent,$(ENTITIES),src/test/$(ent).cpp)
# LIB_SRCS := $(foreach lib,$(LIB_DIRS),$(wildcard $(lib)/src/*.cpp))
...
# LIB_OBJS := $(foreach lib_src,$(LIB_SRCS),$(subst /src/,/obj/,$(dir $(lib_src)))$(notdir $(lib_src:%.cpp=%)).o)
...
.SECONDEXPANSION:
$(TEST_BINS): %: lib/libs.a obj/debug/$$(notdir %.o) obj/test/$$(notdir %.o)
	$(CXX) -o $@ $^ $(LDFLAGS) $(LDLIBS)
